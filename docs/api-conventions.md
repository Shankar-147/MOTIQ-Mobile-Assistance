...
| `GET\|POST /api/v1/vehicles`, `PATCH\|DELETE /api/v1/vehicles/:id` | Customer (own) | CRUD on the customer's own `Vehicle` records — used by the mobile first-login onboarding step and the Profile "My Vehicles" section. `RequestService` still owns the read-only snapshot copied onto a `ServiceRequest` at creation time, so editing/deleting a `Vehicle` never changes a historical request. |
...
| `GET\|POST /api/v1/provider-fleet-vehicles`, `PATCH\|DELETE /api/v1/provider-fleet-vehicles/:id` | Provider (own) | CRUD on the provider's own `ProviderFleetVehicle` records — the fleet-vehicle equivalent of `/vehicles`, used by the Profile "My Fleet" section. No onboarding step (unlike Customer vehicles). |
| `GET /api/v1/providers/me/earnings` | Provider | Earnings summary (`totalEarnings` from `CAPTURED` payments, `pendingAmount` from `PENDING`/`AUTHORIZED`, last 10 `recentPayments`) — derived via the provider's `ACCEPTED` `Assignment`s, since `Payment` has no direct `providerProfileId` column. Lives on `PaymentController` (owns `Payment`), not `ProviderController`. |
| `GET /api/v1/providers/me/ratings` | Provider | Own rating history, cursor-paginated — no customer identity in the payload. Lives on `RatingController` (owns `Rating`), not `ProviderController`. |
