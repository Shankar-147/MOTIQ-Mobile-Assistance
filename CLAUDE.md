...
7. **AI is additive, never load-bearing for the critical path** (ADR 0007). Every AI call site on the critical path (classification, ranking, ETA) needs a deterministic fallback as its designed default, not an error-handler afterthought. The SOS path (Ch55, `SosModule`, ADR 0021) never goes through the AI interface in the primary direction — `AiModule` may call into `SosModule` as a one-way safety-net addition (the emergency pre-filter files a real alert), but `SosModule` never imports `AiModule` and the mobile SOS button has zero AI dependency.
